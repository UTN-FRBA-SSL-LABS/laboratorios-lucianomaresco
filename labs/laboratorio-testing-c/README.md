...
> R: carrito_init inicializa el carrito y lo deja preparado para ser utilizado. Es importante llamarla antes de usarlo para asegurar que el carrito tenga un estado inicial válido y evitar trabajar con datos no inicializados.
...
> R:El bug está en la línea que contiene `total += c->items[i].precio;`. Esa línea suma solamente el precio del producto y no tiene en cuenta su cantidad.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Debería sumar el precio del producto multiplicado por su cantidad: `total += c->items[i].precio * c->items[i].cantidad;`.
